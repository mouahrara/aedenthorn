	...
	public partial class ModEntry
	{
		public class Crop_draw_Patch
		{
			public static void Postfix(Crop __instance, SpriteBatch b, Vector2 tileLocation)
			{
				...
				b.Draw(Game1.objectSpriteSheet, pos, new Rectangle?(Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, int.Parse(__instance.indexOfHarvest.Value), 16, 16)), Color.White * opacity, 0f, new Vector2(8f, 8f), size, SpriteEffects.None, base_sort - 1.1E-05f);
				if (__instance.programColored.Value)
				{
					b.Draw(Game1.objectSpriteSheet, pos, new Rectangle?(Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, int.Parse(__instance.indexOfHarvest.Value) + 1, 16, 16)), __instance.tintColor.Value * opacity, 0f, new Vector2(8f, 8f), size, SpriteEffects.None, base_sort - 1E-05f);
				}
			}
		}
	}
